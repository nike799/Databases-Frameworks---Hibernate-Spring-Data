...
public interface ProductRepository extends JpaRepository<Product,Long> {
    List<Product> findAllByPriceIsBetweenAndBuyerIsNullOrderByPriceAsc(BigDecimal after, BigDecimal before);
    @Query(value = "select p from productsshop.domain.model.Product p where p.seller is not null and p.buyer is not null order by " +
            "p.seller.firstName")
    List<Product> findAllByAtLeastOneProductSoldWithTheirBuyer();
}
